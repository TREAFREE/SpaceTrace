...
- **Local engineering RC generation: CONDITIONAL GO.** Maintainers may generate `0.1.0-rc.4` ad-hoc artifacts to continue controlled testing. They are not Apple-verified, are expected to be rejected by Gatekeeper, and are limited to Apple Silicon with a declared—not runtime-qualified—macOS 15.6 minimum.
...
- Two independent `rc.4` packages from pushed commit `4ad9ca9` passed four-entry checksums, strict signing, exact-entitlement, final dependency audit, arm64, deployment-target, read-only DMG, manifest, SPDX/notices, fresh/same-build/replacement process, and quarantined-copy checks. Gatekeeper and distribution policy rejected the candidate for the disclosed ad-hoc and missing-notary reasons.
