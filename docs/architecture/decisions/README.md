...
| [ADR-008](ADR-008-append-only-reconciliation-corrections.md) | Append-only provisional-history revisions and registered same-evidence correcting projections | Proposed; schema-v12 implementation, migration, benchmark, terminal queries, status UI, and one current-host 5 GiB trial complete; repeated KPI, maintainer adoption, and release gates pending |
