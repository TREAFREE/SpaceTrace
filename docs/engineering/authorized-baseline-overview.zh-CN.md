...
本次实现把已经恢复的用户自选目录接入可取消的元数据基线扫描，并让概览页只展示可验证的结果。已提交记录包含启动数据卷容量快照，并能在 App 重启后恢复。权限 UI 会独立投影全部已配置 scope，并把当前所有活跃 scope ID 交给有界多根目录请求。结合已经实现的扫描调度生命周期和持久化历史/finding 流程，这满足 FR-002 的基线契约：中断扫描要么从持久 dirty work 安全重启，要么不新增“完整”基线。继续使用 `FileManager` 枚举器的内存偏移不是 PRD 要求，它仍可作为非 P0 性能增强，但不是发布阻塞项。
...
上下文提供器只接受已经活跃的 `WatchedScopeID`，返回精确恢复的根目录以及当前挂载 generation 的 stream ID。UI 绝不会根据路径文本构造权限能力或 stream 身份。每个新添加目录都会获得与路径无关的持久 ID；重新授权和更换目录继续复用该 ID。Capability catalog 会拒绝完全相同的重复根目录。一次请求只能包含 1–64 个互不重复的 scope ID，开始工作前会按稳定 ID 排序。
...
SQLite 启动时会执行一个有界恢复事务。所有仍为 `running` 的校准行都属于已经结束的旧进程：删除其 staging、标记为失败，但保留 durable dirty work。SpaceTrace 不会声称可以从只存在于内存中的任意文件枚举中点继续扫描。FR-002 允许安全重启，本实现采用的正是这个分支：丢弃未完成 staging、保留 durable dirty work，并在条件允许时重新扫描受影响根目录。目录授权恢复后，协调器只读取包含该 scope 的最近已提交基线，并在 UI 中标为“从本机记录恢复”。
...
当一个或多个目录授权和监控代次均已就绪时，概览页会针对完整活跃集合提供“开始基线扫描”。暂不可用或 stale 的条目会保持独立可见，不会隐藏健康授权。扫描期间会展示当前类型化阶段、已用时间、根目录计数、已经可获得的扫描计数，以及取消按钮。

成功卡片会展示：

- 每个精确的已授权根目录各自独立展示，不伪造可能因路径重叠而重复计算的字节总量；
- 完整覆盖状态；
...
确定性的 package 测试覆盖完整发布、部分覆盖不发布、revision 被取代、取消、类型化阶段顺序、上下文失败、容量采样、多根目录确定性顺序、单快照提交、后续根部分失败、后续根取消、请求边界、重复根目录拒绝、快照往返、重启恢复、中断 staging 清理，以及 dirty work 保留。应用测试覆盖 MainActor 多 scope 投影、稳定的新增/重新授权/移除命令、混合可用状态、变更失败后保留可信状态、请求上限、批量转发与恢复。完整仓库门禁会构建 Debug/Release App，并运行 package 与应用单元测试。

原生 security-scoped 选择和挂载生命周期继续由已有的签名沙盒与 APFS 镜像规程覆盖。多 scope UI 测试 target 已完成无签名编译，但当前钥匙串没有有效 Apple 代码签名身份，因此不声明交互式 UI 测试已经运行通过。本次实现也不声明完整 FR-002 旅程已经在 macOS 15.6 上通过；在更新主机上按 deployment target 编译不等于真实运行资格验证。
