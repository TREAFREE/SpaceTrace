...
本次实现把已经恢复的用户自选目录接入可取消的元数据基线扫描，并让概览页只展示可验证的结果。已提交记录包含启动数据卷容量快照，并能在 App 重启后恢复。权限 UI 现在会独立投影全部已配置 scope，并把当前所有活跃 scope ID 交给有界多根目录请求。FR-002 **尚未**实现真正的扫描中点续扫、温度与电源调度，以及历史比较点。
