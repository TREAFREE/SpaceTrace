...
| 主机 | 时长 | 必需转换 | 状态 |
| --- | ---: | --- | --- |
| Apple Silicon 当前稳定版 macOS | 至少 24 小时 | 启动、普通运行、真实睡眠/唤醒、时间变化、时区变化、跨本地午夜、退出/重启 | 2026-07-30 ad-hoc 耐久运行通过；明确的时间/时区变化、签名状态项和 Release Candidate 替换行仍待完成 |
...
## 2026-07-30 当前主机重跑：通过

加固后的 runner 于 2026-07-30 16:00:58 UTC 启动提交 `ed0d660`，使用独立 bundle identifier、ad-hoc 签名、Release 配置和 App Sandbox，在 Apple Silicon macOS 26.5.2 上运行，并于 2026-07-31 17:09:52 UTC 自动结束。预检验证了复制后的运行 bundle 与 `LSMinimumSystemVersion = 15.6`。它仍然只属于当前主机工程证据，不属于 Developer ID、公证、分发或 macOS 15.6 真实运行证据。

终态产物为 `PASSED`：采集失败为 0，禁止字段扫描为空，分析器退出码为 0。单一 session 生成 794 条无路径记录，共覆盖 90,529,656 ms（25 小时 8 分 49.656 秒）。默认策略没有发现问题：

| 分析器指标 | 实测证据 | 预算 |
| --- | ---: | ---: |
| 最大清醒 heartbeat 间隔 | 62,097 ms | 300,000 ms |
| 最大唤醒恢复时间 | 1 ms | 10,000 ms |
| 最大常驻内存 | 139,984,896 字节 | 150,000,000 字节 |
| 最大数据库聚合大小 | 613,696 字节 | 250,000,000 字节 |
| 平均 CPU | 0.011710% | 0.5% |
| 区间 p95 CPU | 0.039200% | 2% |

运行中同时观察到成功 retention 和最终 qualified 容量端点。受保护诊断目录约 452 KiB，目录/文件权限保持 `0700`/`0600`，且没有路径、bookmark、卷身份、容量具体值、环境、命令行或文件名字段。

五次 Activity Monitor 采集请求及其 ledger/live/thermal 导出全部成功。运行结束后新增的类型化分析器会解析 xctrace `id`/`ref` 单元格，并在缺少配套表、累计计数器回退或 ledger 不一致时失败关闭。对保留导出的重新分析生成 3,457 字节受保护 JSON，SHA-256 为 `759627dd8ad595d79a06821cfefd2e0d192ed67d50f562d495fc63289920bc44`：

| Instruments 指标 | 实测证据 |
| --- | ---: |
| 采集区间 CPU 时间 | 0.401045252 秒 |
| 各切片平均 CPU | 0.011421%–0.033809% |
| 各切片 p95 CPU | 0.022102%–0.126791% |
| 最高瞬时 CPU | 1.507951% |
| Idle Wake Ups | 959 次 |
| 磁盘写入 / 读取 | 811,008 / 352,256 字节 |
| 最大物理内存 footprint | 99,271,664 字节 |
| App Nap | 5 个切片中的 3 个观察到 |
| Preventing Sleep | 从未观察到 |
| Thermal State | 5 个切片均为 Nominal |

五次名义上各 5 分钟的采集请求，其导出 live 区间包含系统睡眠造成的间隔；因此分析器依据真实行与累计计数器差值计算，不假定总墙上时间恰好为 25 分钟。这些仍是与能耗有关的进程资源测量，不是瓦特/焦耳测量。同一证据目录明确记录：Power Profiler 拒绝 macOS target，非特权 `powermetrics` 需要交互式管理员授权。至此当前主机 ad-hoc 24 小时门禁关闭；直接功耗、Apple 身份、Release Candidate 和 macOS 15.6 运行门禁仍保持开放。

2026-08-10 又使用独立 bundle identity 的 ad-hoc Release/App Sandbox 完成 60 秒回归，真实经过新的双分析器最终化路径：终态为 `PASSED`，5 条无路径记录覆盖 59,392 ms，一段 Activity Monitor 导出完整；采集失败为 0、隐私扫描为 `PASS`、两个分析器退出码均为 0、报告权限均为 `0600`，且 App 与 launchd job 均无残留。它只验证新版 runner 接线，不替代任何耐久或发布矩阵行。
