...
本阶段完成的是分类引擎和首版回归语料。纯不可变历史 finding 投影现在会冻结每个可用端点所使用的准确 catalog/规则决策；生产 SQLite 持久化、概览 UI、“至少 60 个已知场景”语料门禁以及由用户控制的诊断导出仍未实现。
...
社区规则包、本地化证据文案、生产 finding 持久化/UI 与显式重新计算属于后续受治理功能，不是 catalog v1 的隐含行为。
