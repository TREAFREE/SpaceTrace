...
本阶段已经完成分类引擎和经过审阅的第二版回归语料。不可变历史 finding 流水线会冻结每个可用端点所使用的准确 catalog/规则决策，将其持久化到 SQLite，并在有界概览中原样展示而不重新分类。FR-007/KPI-03 的仓库语料门禁现已关闭；由用户控制的诊断导出仍未实现。
...
`attribution-fixtures-v2.json` 包含 64 个已知场景（8 个 P0 类别各 8 个）和 32 个近似/Unknown 场景。单独的规则契约表冻结了每条内置规则的 ID、规则版本、类别、置信度和无路径证据代码；每个已知样本都指定准确的预期规则，而不只是类别。当前已提交的合成审阅语料上，precision、recall 与 Unknown accuracy 都是 100%。测试还强制 case ID 唯一且只使用小写 ASCII、catalog 覆盖准确，并覆盖组件边界、大小写、home directory 以及显式快照上下文反例。独立测试继续证明跨类别并列会保持歧义，并且三种指标使用不同分母。原始 v1 语料仍作为历史证据保留，但不会被计入 v2 分母来放大样本数。

这只是回归证据，不是真实用户分布或准确率证据。它已经满足 KPI-03“至少 60 个已知路径且 precision 不低于 95%”的仓库门禁，也满足每类 8 个样本的更强审阅目标，并且不会给 32 个 Unknown 样本强行贴标签。已冻结的 finding 决策会被持久化并原样展示。Public Beta 仍由最低系统、人工辅助功能/可用性、导出/脱敏、治理和分发等独立门禁阻断。
...
社区规则包、本地化证据文案、replacement/supersession 与显式重新计算属于后续受治理功能，不是 catalog v1 的隐含行为。
