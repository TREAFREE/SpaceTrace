...
3. complete packaged export, signed-sandbox restart/revocation/external-volume, clean quarantine, macOS 15.6, distribution-trust, and maintainer-acceptance gates.
...
Repository completion still requires strict-concurrency verification, `make verify`, `git diff --check`, and a privacy scan. Those checks cover the production paired-v11, explicit-disappearance, stable-APFS-move, reviewed classifier-corpus, and automated Overview slices but do not substitute for unfinished signed-sandbox/minimum-OS qualification, manual UI review, export, or distribution gates above.
