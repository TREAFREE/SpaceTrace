...
| Non-UI monitoring composition | Actor-isolated Disk Arbitration consumption, bounded mount-readiness retry, transactional activation/closure, one owned FSEvents consumer task per scope, generation-conditional stop/restart, post-start recovery with exponential backoff and circuit breaking, recovery observability, and overflow-driven source recreation | Native callback order cannot silently reorder concurrent generation work; transient failures are retried within fixed bounds; unmount/replacement cancels owned recovery; and loss of callback precision closes correlated streams before re-enumeration |
...
- `swift test --package-path Packages/SpaceTraceKit`: 131 tests in 20 suites (the destructive-environment qualification remains opt-in and is reported as skipped here);
...
1. Add genuine daemon drop/wrap qualification where safely reproducible and property-based mount/event state sequences.
