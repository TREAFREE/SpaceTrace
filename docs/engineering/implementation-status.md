...
| Calibration pipeline | Actor-isolated ingestion and bounded reconciliation orchestration behind an application-owned scanner port | Partial scans retain dirty work; a completed stale scan cannot clear work updated while it was running |
...
| SQLite prototype | Actor-owned SQLite3 connection, WAL, schema v2, big-endian `UInt64` cursors and revisions, atomic coalescing/checkpoint transaction, cursor-free calibration markers, and rollback fault seam | Durable cursor advancement is atomic with dirty-region persistence, cannot regress, and scan finalization is revision-conditional |
...
- `swift test --package-path Packages/SpaceTraceKit`: 63 tests in 11 suites;
...
- The application pipeline owns a scanner port, but the production metadata enumerator and atomic observation staging/finalization are not implemented yet.
...
1. Implement the bounded metadata enumerator and staged observation finalization behind `CalibrationScanner`, with permission and cancellation coverage.
...
5. Expand the schema migration fixture matrix and add disk-full/corruption tests, retention behavior, and oldest-supported-OS qualification.
