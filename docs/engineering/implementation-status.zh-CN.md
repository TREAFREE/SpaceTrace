...
- 原生 SQLite 适配器现已覆盖迁移前原子备份、启动只读恢复、主库/WAL 隔离、v6/v7 golden fixture、小时/日历史、老化 dirty path 去路径化，以及可复现的 50 万/100 万行当前主机 benchmark。GRDB 行为等价、更深的页级损坏、公证和最低系统运行/性能证据仍未完成。
