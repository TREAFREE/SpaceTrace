...
- 尚未实现面向用户的基线扫描、历史、增长解释、菜单栏 24 小时指标或导出流程。普通主窗口外壳、权限感知概览、目录授权目的地和轻量菜单栏入口已经实现，并且刻意不展示虚构的存储测量。
...
1. 把已授权 scope 的基线流水线接入可取消的应用用例和类型化进度快照，再让概览只渲染已经发布且带 coverage 的真实结果。
...
7. 在相应 ADR 被接受之前，继续保证所有架构验证代码都无法从用户可见流程触达。
