...
以下门禁截至 2026-08-11 已使用 Swift 6.2.1 与 Xcode 26.1.1 通过：

- `swift test --package-path Packages/SpaceTraceKit`：285 个测试、45 个 suite（会改变测试环境的资格测试保持 opt-in，常规运行中显示为 skipped）；
- 同一套 package 测试在完整严格并发诊断以及“编译器警告视为错误”条件下通过；
- 分类模块的 6 个 suite、31 个聚焦测试已通过，覆盖词法路径校验、精确组件匹配、priority/具体度顺序、同类别稳定并列、跨类别歧义、明确快照上下文、8 个首版 P0 类别、24 个已知 fixture、8 个 Unknown 近似反例，以及相互独立的 precision/recall/Unknown 分母检查；
...
- 面向用户的多目录权限列表、批量基线、启动卷容量历史、版本/Schema 元数据、重启恢复、感知电源/温度/睡眠的暂停与重试、schema v10 睡眠感知目录/卷历史概览、具备资格判断的菜单栏 24 小时指标以及纯确定性分类器均已实现。分类器尚未接入 finding、持久化或 UI，24 个已知/8 个 Unknown 的合成语料也低于 PRD 门禁；APFS 唯一物理块核算、移动/删除 finding、真正从枚举器内存中点续扫和导出尚未实现。
...
8. 把已审阅分类语料从当前 24 个已知场景扩充到 PRD 门禁，再把精确规则版本/证据接入应用 finding、持久化与概览，并禁止静默重算历史文案。
...
- 阻塞门禁包括：分类 finding/持久化/UI 接入与语料扩充、尚未完成的移动删除/export P0 行为与导出脱敏证据；macOS 15.6 真实运行；人工辅助功能与规定的可用性测试；真实权限/替换 bookmark 矩阵；ADR-003/ADR-004 接受；Developer ID/公证，或项目所有者明确接受未签名风险并完成干净 quarantine 安装；打包升级/回滚；正式许可、第三方 notices 与 SBOM。
