...
- 面向单个已授权目录的基线、启动数据卷容量采样、版本/Schema 元数据、重启恢复以及有界的应用层多根目录调度均已实现。权限 UI 仍只展示一个主目录；真正的扫描中点续扫与电源/温度调度不在本切片范围内。历史、增长解释、菜单栏 24 小时指标和导出也尚未实现。
...
1. 将权限 UI 的已授权目录列表接入有界多根目录请求，并补齐用户可见的聚合进度、移除行为和批量重扫入口；不得把重启后恢复描述成扫描中点续扫。
