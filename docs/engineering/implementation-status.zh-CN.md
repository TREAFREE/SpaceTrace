...
| 应用进程生命周期 | AppKit 启动/退出桥接；Application Support 组合；catalog 恢复；唯一 runtime 任务；唯一扫描调度 monitor；一个启动后立即执行并按小时运行的启动卷采样任务；异步退出协调；runtime 关闭后释放 lease | 关闭窗口不会停止监控；退出时会在释放权限能力前取消所属监控、调度、扫描与容量任务；卷容量采样不依赖目录授权 |
...
- `swift test --package-path Packages/SpaceTraceKit`：211 个测试、32 个 suite（会改变测试环境的资格测试保持 opt-in，常规运行中显示为 skipped）；
...
- 确定性的已授权目录基线与历史测试证明类型化阶段顺序、取消完成、部分覆盖/被取代结果不发布、权限切换前取消、从真实 SQLite 读回完整根目录聚合、独立历史曲线、明确缺口、根目录受限增长以及跨 scope 拒绝；同时 19 个应用单元测试通过，覆盖 MainActor 多 scope 投影、批量命令转发、恢复后的历史上下文、窗口切换、失败/取消状态，以及不可用时间桶处的图表断线；
...
- 面向用户的多目录权限列表、批量基线、启动卷容量历史、版本/Schema 元数据、重启恢复、感知电源/温度/睡眠的暂停与重试，以及 schema v9 目录/卷历史概览均已实现。归因只使用互不重叠授权根 allocated-size 增长，是有证据边界的近似；APFS 唯一物理块核算、确定性分类、移动/删除 finding、真正从枚举器内存中点续扫、菜单栏 24 小时指标和导出尚未实现。
