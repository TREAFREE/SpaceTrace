...
| 后台空间生命周期与菜单栏 | 启动/唤醒/系统时间变化后立即采样；睡眠感知延后；系统每天提供机会且 single-flight 的 retention；有界后台健康状态；按提交序号判断 24 小时资格；`MenuBarExtra` 展示当前容量、最近采样/校准和类型化的不完整状态 | 采样与 retention 不会重叠；缺口、过期端点、时间回拨、换卷、有界查询截断或指标不可用都会取消差值；刷新失败时不能继续把旧结果当作当前结果 |
| 后台 soak 诊断 | 明确开启的应用层 recorder；无路径生命周期/资源 Schema；每次启动随机 session ID；最长 7 天/最多 10 MiB 且受保护的两段式 JSONL；原生 CPU/内存/数据库聚合探针；确定性 24 小时分析器；菜单栏可见记录提示 | 默认运行不写资格日志；诊断结构无法表达路径、卷身份、容量具体值或 bookmark；日志失败不影响监控；smoke 策略不能生成默认 24 小时通过结论 |
...
| SQLite 原型 | actor 持有的 SQLite3 连接；WAL；schema v9；有界 opaque security-scoped bookmark；大端 `UInt64` 游标/revision；持久 scope 挂载 generation；扫描运行/目录 staging 表；当前/已删除目录聚合；已提交多根基线；小时/日目录历史；单调启动卷容量历史；根目录受限增长查询；原子发布；无游标/无路径校准标记；checkpoint generation 作废；类型化磁盘写满/损坏/迁移错误；v6/v7/v8 golden fixtures；有界 retention | 游标推进与 dirty work 原子且不能倒退；基线/容量发布保持原子；系统时间回拨不能重排容量提交；写满失败保留已提交状态；损坏不被静默替换；迁移失败回滚 schema/version；retention 移除老化历史，同时保留当前事实、未解决无路径工作和每个 scope 最新基线 |
...
- `swift test --package-path Packages/SpaceTraceKit`：237 个测试、37 个 suite（会改变测试环境的资格测试保持 opt-in，常规运行中显示为 skipped）；
...
- 面向用户的多目录权限列表、批量基线、启动卷容量历史、版本/Schema 元数据、重启恢复、感知电源/温度/睡眠的暂停与重试、schema v9 目录/卷历史概览，以及具备资格判断的菜单栏 24 小时指标均已实现。归因只使用互不重叠授权根 allocated-size 增长，是有证据边界的近似；APFS 唯一物理块核算、确定性分类、移动/删除 finding、真正从枚举器内存中点续扫和导出尚未实现。
- 30 个虚拟日的后台资格测试以及明确开启、有界的诊断/分析器基础设施属于确定性逻辑证据，不等于已经完成真实进程连续运行 24 小时、系统调度到达保证、能耗结果、签名状态项交互矩阵或 macOS 15.6 运行资格。内部累计 CPU 时间不是能耗测量。
...
- 原生 SQLite 适配器现已覆盖迁移前原子备份、启动只读恢复、主库/WAL 隔离、v6/v7/v8 golden fixture、小时/日目录历史、单调容量历史、老化 dirty path 去路径化，以及可复现的 50 万/100 万行当前主机 benchmark。GRDB 行为等价、更深的页级损坏、公证和最低系统运行/性能证据仍未完成。
...
1. 使用签名 sandbox App，在 macOS 15.6 与当前稳定版 macOS 上完成真实 24 小时运行、睡眠/唤醒、时间/时区变化、交流电/电池、低电量模式和安全可控温度转换的生产信号资格验证；同时验证 retention 到达、菜单栏资格/降级、能耗、内存和数据库增长。只有具备 benchmark 证据后才增加后台 token bucket/速率策略。
