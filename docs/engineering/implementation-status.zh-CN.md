...
| FSEvents 桥接层 | 按设备解析监控目标；持久化卷/日志标识；校验卷内相对路径；还原应用绝对路径；支持重启的完整历史回放；可注入故障的流构造；当持久历史回放被拒绝时执行带校准的一次性恢复；处理根目录变化哨兵；安全持有原生资源生命周期；有界单消费者适配器；无日志 UUID 卷使用仅实时的主机流降级方案 | 持久回放同时绑定卷 UUID 与 FSEvents 日志 UUID；绝不持久化临时的 `dev_t`；存量回放被拒绝时，必须先原子作废 checkpoint 再开始实时监控；非持久降级流只属于一个挂载 generation |
...
| 非 UI 监控组合 | actor 隔离的 Disk Arbitration 消费；有界挂载就绪重试；事务型激活/关闭；每个 scope 只拥有一个 FSEvents 消费任务；按 generation 条件停止/重启；溢出后重建事件源 | 原生回调顺序不能悄悄重排并发 generation 工作；短暂的挂载可见性问题只在固定上限内重试；回调精度丢失时，必须在重新枚举前关闭相关事件流 |
...
- `swift test --package-path Packages/SpaceTraceKit`：125 个测试、20 个 suite（会改变测试环境的资格测试保持 opt-in，常规运行中显示为 skipped）；
...
- 同步流启动恢复已经有界且经过测试。流启动后若意外失败，supervisor 会记录失败但尚不会自动重开；该终止条件的生命周期级恢复仍是验收项。
...
1. 为启动后事件流意外终止增加生命周期恢复；在可安全复现时验证真实守护进程 drop/wrap；加入基于性质的挂载/事件状态序列测试。
