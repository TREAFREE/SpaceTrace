...
| 已授权目录基线应用流程 | 精确解析已授权 scope/活跃 stream 上下文；actor 持有可取消任务；类型化的准备/扫描/发布/完成状态；权限切换时取消；概览页按覆盖状态投影 | UI 字节结果只能来自按 revision 校验并原子发布后重新读回的完整根目录聚合；部分覆盖、取消和被取代的尝试绝不能成为完整事实 |
...
- `swift test --package-path Packages/SpaceTraceKit`：161 个测试、25 个 suite（会改变测试环境的资格测试保持 opt-in，常规运行中显示为 skipped）；
...
- 确定性的已授权目录基线测试证明类型化阶段顺序、取消完成、部分覆盖/被取代结果不发布、权限切换前取消，以及从真实 SQLite 读回完整根目录聚合；同时 9 个应用单元测试通过，其中包括 MainActor 基线状态投影；
...
- 面向单个已授权目录的基线已经实现，但完整 FR-002 仍未完成：启动数据卷容量/可用空间采样、多根目录、可恢复续扫以及电源/温度调度仍待实现。历史、增长解释、菜单栏 24 小时指标和导出也尚未实现。
...
1. 完成 FR-002 剩余基线记录：启动数据卷容量/可用空间采样、版本/schema 元数据、多根目录进度以及安全的重启/续扫策略。
...
6. 增加温度、电源、睡眠/唤醒、权限撤销以及生产校准调度策略。
