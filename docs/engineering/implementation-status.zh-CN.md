...
以下门禁截至 2026-07-24 已使用 Swift 6.2.1 与 Xcode 26.1.1 通过：

- `swift test --package-path Packages/SpaceTraceKit`：229 个测试、35 个 suite（会改变测试环境的资格测试保持 opt-in，常规运行中显示为 skipped）；
...
- 确定性的已授权目录基线与历史测试证明类型化阶段顺序、取消完成、部分覆盖/被取代结果不发布、权限切换前取消、从真实 SQLite 读回完整根目录聚合、独立历史曲线、明确缺口、根目录受限增长以及跨 scope 拒绝；同时 23 个应用单元测试通过，覆盖 MainActor 多 scope 投影、批量命令转发、恢复后的历史上下文、窗口切换、失败/取消状态、不可用时间桶处的图表断线，以及菜单栏证据投影；
...
- 30 个虚拟日的后台资格测试属于确定性的逻辑证据，不等于真实进程连续运行 24 小时、系统调度到达保证、能耗/内存结果、签名状态项交互矩阵或 macOS 15.6 运行资格。
