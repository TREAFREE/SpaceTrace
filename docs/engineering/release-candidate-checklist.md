...
|---|---|---|
| Fresh copy from mounted DMG | App launches only after the disclosed per-app Gatekeeper exception; no fabricated history | Read-only mount, copy, quarantine, and expected Gatekeeper rejection passed for `0.1.0-rc.7`; clean-account **Open Anyway** launch remains open |
...
| Replace Applications copy with a separately built RC | App launches; existing bookmark either restores exactly or presents explicit reauthorization | Non-quarantined `rc.6 → rc.7` and `rc.7 → rc.6` process replacements passed; packaged bookmark continuity remains open and must not be assumed from ad-hoc signing |
...
The retained `0.1.0-rc.7` artifact was built on macOS 26.6.1 with Xcode 26.1.1 from clean, pushed commit `a3b8b45ca6f9dad752a9b750422b343d71c04dc8`. Its packaging contract passed signature, exact-entitlement, architecture, deployment-target, final Mach-O dependency, DMG, manifest, four-entry checksum, SPDX, and notices verification. The executable SHA-256 is `f52a0ff3984256e606f35c61b400b2da76283b629fa315a34667b38858da356a`; the DMG SHA-256 is `dab727280f2c68829cc4ddb51b78bebe1ba1250cc3c6433d4dcf0fea8116d38a`; the SPDX SHA-256 is `53f7e468c9754919a282418e0f86ef23682586f2c004841cb933246a2e228a46`; and the notices SHA-256 is `c5bfed60122264ac1123eeabeb61893e94b964b01e1eb8fd4c79fcf5b83f4250`. The prior checksummed `rc.6` artifact provided an independently built binary for the forward and reverse process-replacement rows; no byte-identical claim is made.
...
The packager now creates `READ-ME-FIRST.txt` through one deterministic version/source-bound generator. The integration test and final readiness gate independently regenerate it and compare the mounted bytes exactly. The bilingual notice includes the checksum and manifest commands, drag-to-Applications and **Open Anyway** journey, no-global-bypass rule, manual-update/rollback limits, and the no-cleanup/no-deletion product boundary.
...
Using non-quarantined retained Apps under disposable Bundle IDs, both `rc.6 → rc.7` and `rc.7 → rc.6` completed fresh launch, graceful same-build restart, replacement launch, and graceful quit. No directory was selected, so this is process replacement rather than bookmark or database rollback qualification. A copied `rc.6` carrying synthetic quarantine was launched once during earlier root-cause investigation: macOS created the process and then Apple System Policy terminated it, exactly matching the disclosed trust boundary. The qualifier rejects such input before launch. macOS containermanager privacy denied the system `trash` operation for each approximately 984 KiB disposable container, so cleanup is recorded as blocked rather than passed. The five current-host residuals created while qualifying the rc.6 and rc.7 boundary are:
