...
| Current-host controlled UI/accessibility tree | Passed for automated fixture scope | Fourteen authorization/history/finding/export scenarios passed; the real signed-sandbox save panel produced a bounded redacted JSON file and passed three consecutive repetitions. Manual VoiceOver, Full Keyboard Access, contrast/motion/larger-text review remains open in the [visual design system](./visual-design-system.md) |
| Ad-hoc RC artifact contract | Passed | Two independent `0.1.0-rc.4` builds from pushed commit `4ad9ca9` passed four-entry checksums, strict code signing, exact entitlements, final dependency audit, architecture, deployment target, read-only DMG, manifest, SPDX, and notices checks; a quarantined copy was rejected by Gatekeeper and distribution policy exactly as disclosed |
| Complete P0 product workflow | Open — release blocking | FR-002's safe restart/no-false-complete branch is implemented; in-enumerator midpoint continuation is a non-P0 performance enhancement, not a release requirement. The deterministic classifier, reviewed 64-known/32-Unknown FR-007 corpus, immutable coverage-aware finding projection, schema-v11 persistence, complete-parent disappearance reconciliation, qualified APFS stable moves, projector lifecycle, finding/History-Off Overview, and user-controlled redacted diagnostic export are implemented. FR-004 correction/replacement remains open; external qualification is tracked separately below |
...
| Fresh quarantined clean-account install | Partial — release blocking | The `rc.4` DMG mounted read-only, copied correctly, retained strict code-sign validity under quarantine, and was rejected for the disclosed ad-hoc/notary reasons; a clean-account per-app Gatekeeper exception and first-launch journey remain open |
| Update/replacement and bookmark continuity | Partial — release blocking | `rc.4` fresh/same-build/independent-build process launch passed under a disposable ID; no bookmark was created, so cross-build restore/reselection behavior remains open |
